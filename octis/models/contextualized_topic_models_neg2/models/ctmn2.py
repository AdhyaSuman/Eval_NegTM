...
from octis.models.contextualized_topic_models_neg2.networks.decoding_network import DecoderNetwork
...
class CTMN2(object):
    ...
    def __init__(self, input_size, bert_input_size, inference_type="zeroshot", num_topics=10, model_type='prodLDA',
                 ...
                 topic_perturb=1, tloss_weight=1):  #DKS: Last 2 parameters added
        """
        ...
        self.model = DecoderNetwork(
            ...
            dropout, self.learn_priors, self.topic_prior_mean, self.topic_prior_variance, topic_perturb=self.topic_perturb)
        ...
        self.margin = 1               #DKS
        self.triplet_loss = nn.TripletMarginLoss(margin=self.margin, p=2) #DKS
        self.neg_method = 2           #DKS
        self.triplet_loss_weight = tloss_weight


    def _loss(self, inputs, word_dists, prior_mean, prior_variance,
              posterior_mean, posterior_variance, posterior_log_variance, word_dists_neg):
        # KL term
        # var division term
        ...
        #Find the triplet loss (TL)
        #anchor: word_dists, +ve example: inputs, -ve example: word_dists_neg
        if (self.neg_method==2):
            #triplet_loss(anchor, positive, negative)
            #word_dists_trimmed = torch.where(inputs>0, word_dists, 0.)
            #word_dists_neg_trimmed = torch.where(inputs>0, word_dists_neg, 0.)
            #print("Doc lengths: ", torch.count_nonzero(inputs, dim=-1))
            #print("inputs: ", inputs)
            #print("Word dists: ", word_dists_trimmed)
            #print("word_dists_neg: ", word_dists_neg_trimmed)
            TL = self.triplet_loss(word_dists, inputs, word_dists_neg) #DKS: TESTED, RECONSTRUCTED == ANCHOR
            #TL = self.triplet_loss(word_dists_trimmed, inputs, word_dists_neg_trimmed) #DKS: TESTED, RECONSTRUCTED == ANCHOR
            #TL = self.triplet_loss(inputs, word_dists_trimmed, word_dists_neg_trimmed) #DKS: INPUT == ANCHOR
        else:
            TL = 0
        
        
        loss = KL + RL + self.triplet_loss_weight * TL


        return loss.sum(), self.triplet_loss_weight * TL
    ...
    def _train_epoch(self, loader):
        ...
        for batch_samples in loader:
            ...
            word_dists, topic_word, topic_document, word_dists_neg = self.model(X, X_bert)
            topic_doc_list.extend(topic_document)

            # backward pass
            loss, TL = self._loss(X, word_dists, prior_mean, prior_variance,
                              posterior_mean, posterior_variance, posterior_log_variance, word_dists_neg)
            loss.backward()
            self.optimizer.step()
            ...
            triplet_loss += TL.item()

        train_loss /= samples_processed
        triplet_loss /= samples_processed

        return samples_processed, train_loss, topic_word, topic_doc_list, triplet_loss


    def _get_validation_loss_npmi (self, texts_valid):

        npmi = Coherence(texts=texts_valid, measure='c_npmi')
    ...
    def _validation(self, loader):
        ...
        for batch_samples in loader:
            ...
            #DKS: Use NPMI instead of loss
            #npmi_score = npmi.score(self.get_info())# CoherenceNPMI(texts=texts, topics=self.get_topic_lists(k=10))
            samples_processed += X.size()[0]
            val_loss += loss.item()
            #npmi_agg += npmi_score
    ...
    def fit(self, train_dataset, validation_dataset=None, save_dir=None, verbose=True, texts=None, gap=None):
        """
        Train the CTM model.
        ...
        if verbose:
            print("Settings: \n\
                ...
                self.lr, self.momentum, self.reduce_on_plateau, save_dir, self.topic_perturb, self.triplet_loss_weight))
        ...
        #npmi = Coherence(texts=texts[0], measure='c_npmi')  #DKS: NPMI on train data
        ...
        for epoch in range(self.num_epochs):
            ...
            sp, train_loss, topic_word, topic_document, triplet_loss = self._train_epoch(train_loader)
            samples_processed += sp
            e = datetime.datetime.now()

            if verbose:
                print("Epoch: [{}/{}]\tSamples: [{}/{}]\tTrain Loss: {}\tTL: {}\tTime: {}".format(
                    epoch + 1, self.num_epochs, samples_processed,
                    len(self.train_data) * self.num_epochs, train_loss, triplet_loss, e - s))
            ...
            if self.validation_data is not None:
                ##s = datetime.datetime.now() ##DKS
                ##val_loss = self._get_validation_loss_npmi (texts_valid = texts[1])
                ##e = datetime.datetime.now()
                ...
                # train epoch
                s = datetime.datetime.now()
                val_samples_processed, val_loss = self._validation(validation_loader) #DKS: Commented
                #val_samples_processed, val_loss = self._validation(validation_loader, texts_valid = texts[1])
                e = datetime.datetime.now()

                if verbose:
                    print("Epoch: [{}/{}]\tSamples: [{}/{}]\tValidation Loss: {}\tTime: {}".format(
                        epoch + 1, self.num_epochs, val_samples_processed,
                        len(self.validation_data) * self.num_epochs, val_loss, e - s)) #DKS
                      
                    
                    #print("Epoch: [{}/{}]\tSamples: [{}/{}]\tValidation NPMI: {}\tTime: {}".format(
                    #    epoch + 1, self.num_epochs, len(self.validation_data),
                    #    len(self.validation_data) * self.num_epochs, -val_loss, e - s)) #DKS        
                ...
                if np.isnan(val_loss) or np.isnan(train_loss):
                    ...
                    if self.early_stopping.early_stop:
                        ...
                        break
            #report NPMI
            #gap = 2 if epoch<10 else 10
            #if texts[0] and ((epoch + 1)%gap==0):
            #    npmi_score = npmi.score(self.get_info())# CoherenceNPMI(texts=texts, topics=self.get_topic_lists(k=10))
    ...
    def predict(self, dataset):
        ...
        with torch.no_grad():
            for batch_samples in loader:
                ...
                # forward pass
                self.model.zero_grad()
                _, _, _, _, _, _, _, topic_document, word_dists_neg = self.model(X, X_bert)
    ...
    def get_info(self):
        ...
        info['topics'] = topic_word    #DKS: this is used by Gensim Coherence model
