...
import torch
import numpy as np
from octis.models.nETM_model import data
from sklearn.feature_extraction.text import CountVectorizer
from torch import nn, optim
from octis.models.nETM_model import netm
from octis.models.base_netm import BasenETM
...
class nETM(BasenETM):
    ...
    def __init__(self, num_topics=10, num_epochs=100, t_hidden_size=800, rho_size=300, embedding_size=300,
                 ...
                 use_validation=False, topic_perturb=1, tloss_weight=1):
        """
        initialization of nETM
    ...
    def set_model(self, dataset, hyperparameters):
        if self.use_partitions and self.use_validation:
            ...
            train_data, testing_data = dataset.get_partitioned_corpus(use_validation=False)
            ...
            self.train_tokens, self.train_counts, self.test_tokens, self.test_counts \
                = self.preprocess(vocab2id, data_corpus_train, data_corpus_test)
        ...
        print('tloss_weight:', self.hyperparameters['tloss_weight'])
        ## define model and optimizer
        self.model = netm.nETM(num_topics=self.hyperparameters['num_topics'], vocab_size=len(self.vocab.keys()),
                             ...
                             enc_drop=self.hyperparameters['dropout'],
                             topic_perturb=self.hyperparameters['topic_perturb']).to(self.device)
    ...
    def _train_epoch(self, epoch):
        ...
        for idx, ind in enumerate(indices):
            ...
            recon_loss, kld_theta, TL = self.model(data_batch, normalized_data_batch)
            total_loss = recon_loss + kld_theta + self.hyperparameters['tloss_weight'] * TL
            total_loss.backward()
            ...
            acc_tl += torch.sum(TL).item()
            cnt += 1
            log_interval = 20
            if idx % log_interval == 0 and idx > 0:
                cur_loss = round(acc_loss / cnt, 2)
                cur_kl_theta = round(acc_kl_theta_loss / cnt, 2)
                cur_tl = round(acc_tl / cnt, 2)
                cur_real_loss = round(cur_loss + cur_kl_theta + cur_tl, 2)

                print('Epoch: {} .. batch: {}/{} .. LR: {} .. KL_theta: {} .. Rec_loss: {}  .. TL: {} .. NELBO: {}'.format(epoch + 1, idx, len(indices),
                                             self.optimizer.param_groups[0]['lr'],
                                             cur_kl_theta, cur_loss, cur_tl, cur_real_loss))
        ...
        if self.valid_tokens is None:
            ...
            with torch.no_grad():
                ...
                for idx, ind in enumerate(indices):
                    ...
                    val_recon_loss, val_kld_theta, TL = self.model(val_data_batch,
                                                               val_normalized_data_batch)

                    val_acc_loss += torch.sum(val_recon_loss).item()
                    val_acc_kl_theta_loss += torch.sum(val_kld_theta).item()
                    val_cnt += 1
                    val_total_loss = val_recon_loss + val_kld_theta + self.hyperparameters['tloss_weight'] * TL
