...
from octis.models.contextualized_topic_models_neg2.datasets import dataset
from octis.models.contextualized_topic_models_neg2.models import ctmn2
from octis.models.contextualized_topic_models_neg2.utils.data_preparation import bert_embeddings_from_list

import os
import pickle as pkl


class CTMN2(AbstractModel):
    ...
    def __init__(self, num_topics=10, model_type='prodLDA', activation='softplus',
                 ...
                 prior_variance=None, hidden_sizes=(100, 100), use_partitions=True, use_validation=False,
                 num_samples=10, inference_type="zeroshot", bert_path="", bert_model="bert-base-nli-mean-tokens",
    ...
    def train_model(self, dataset, hyperparameters=None, top_words=10, save_dir=None):
        ...
        if self.use_partitions and self.use_validation:
            ...
            ##print("### Joined test: ", data_corpus_test)
            ...
            #print("DKS: bert_train_path: ", bert_train_path) 
            ...
            x_train, x_test, x_valid, input_size = self.preprocess(
                ...
                bert_val_path=bert_val_path,
                bert_model=self.hyperparameters["bert_model"])
                
            self.model = ctmn2.CTMN2(input_size=input_size, bert_input_size=x_train.X_bert.shape[1],
            ...
            self.model.fit(x_train, x_valid, verbose=True, save_dir=save_dir, texts=[train, validation, test])
            result = self.model.get_info() #DKS: We will use it for coherence   #DKS
            ...
            self.model.fit(x_train, verbose=True, save_dir=save_dir, texts = train)
            result = self.inference(x_test)
            return result
            ...
            x_train, input_size = self.preprocess(
                self.vocab, train=data_corpus, bert_train_path=self.hyperparameters['bert_path'] + "_trainAll.pkl", 
                bert_model=self.hyperparameters["bert_model"])
            #print("*** DKS: Hyperparameters: topic_perturb = ", self.hyperparameters["topic_perturb"], ", tloss_weight = ", self.hyperparameters["tloss_weight"])

            self.model = ctmn2.CTMN2(input_size=input_size, bert_input_size=x_train.X_bert.shape[1], model_type='prodLDA',
                             num_topics=self.hyperparameters['num_topics'], dropout=self.hyperparameters['dropout'],
                             activation=self.hyperparameters['activation'], lr=self.hyperparameters['lr'],
                             inference_type=self.hyperparameters['inference_type'],
            ...
            self.model.fit(x_train, None, verbose=True, save_dir=save_dir, texts=[dataset.get_corpus(), None, None])
    ...
    def preprocess(vocab, train, bert_model, test=None, validation=None,
        ...
        #print("[DKS] x_train length before transform = ", len(train))
        x_train = vec.transform(train)

        b_train = CTMN2.load_bert_data(bert_train_path, train, bert_model)
        #print("[DKS] b_train length before transform = ", len(b_train))
        #print("[DKS] x_train length before transform = ", (x_train.to_array()).shape[0])
        ...
        if test is not None and validation is not None:
            ...
            b_test = CTMN2.load_bert_data(bert_test_path, test, bert_model)
            test_data = dataset.CTMDataset(x_test.toarray(), b_test, idx2token)

            x_valid = vec.transform(validation)
            b_val = CTMN2.load_bert_data(bert_val_path, validation, bert_model)
