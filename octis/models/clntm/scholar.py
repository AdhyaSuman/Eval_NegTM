...
class Scholar(object):
    ...
    def __init__(self, config, alpha=1.0, learning_rate=0.001, init_embeddings=None, update_embeddings=True,
        ...
        elif model == 'DecoderNegScholar':
            self._model = DecoderNegScholar(config, self.alpha, update_embeddings, init_emb=init_embeddings, bg_init=init_bg, device=self.device, classify_from_covars=classify_from_covars).to(self.device)
            self.vae_dns = True
    ...
    def fit(self, X, Y, PC, TC, eta_bn_prop=1.0, l1_beta=None, l1_beta_c=None, l1_beta_ci=None):
        ...
        if self.vae_dns:
            return loss.to('cpu').detach().numpy(), Y_probs, thetas.to('cpu').detach().numpy(), nl.to('cpu').detach().numpy(), kld.to('cpu').detach().numpy(), tl.to('cpu').detach().numpy()
        else:
...
class DecoderNegScholar(nn.Module):

    def __init__(self, config, alpha, update_embeddings=True, init_emb=None, bg_init=None, device='cpu', classify_from_covars=False):
        super(DecoderNegScholar, self).__init__()
        ...
        self.l1_beta_ci_reg = config['l1_beta_ci_reg']
        self.l2_prior_reg = config['l2_prior_reg']
        self.topic_perturb = config['topic_perturb']
        self.device = device
        self.classify_from_covars = classify_from_covars
        ...
        self.triplet_loss_weight = config['tloss_weight']
    ...
    def forward(self, X, Y, PC, TC, compute_loss=True, do_average=True, eta_bn_prop=1.0, var_scale=1.0, l1_beta=None, l1_beta_c=None, l1_beta_ci=None):
        ...
        # Obtain -ve topic
        theta_neg = DecoderNegScholar.perturbTheta(theta, self.topic_perturb)

        # combine latent representation with topics and background
        # beta layer here includes both the topic weights and the background term (as a bias)
        eta = self.beta_layer(theta)

        eta_neg = self.beta_layer(theta_neg)
        ...
        eta_neg_bn = self.eta_bn_layer(eta_neg)
        #eta_bn = eta
        ...
        #compute -ve X recon
        X_recon_neg_bn = F.softmax(eta_neg_bn, dim=1)
        X_recon_neg_no_bn = F.softmax(eta_neg, dim=1)
        X_recon_neg = eta_bn_prop * X_recon_neg_bn + (1.0 - eta_bn_prop) * X_recon_neg_no_bn
        ...
        if compute_loss:
            return theta, X_recon, Y_recon, self._loss(X, Y, X_recon, Y_recon, prior_mean, prior_logvar, posterior_mean_bn, posterior_logvar_bn, do_average, l1_beta, l1_beta_c, l1_beta_ci,
                                                       X_recon_neg=X_recon_neg)
        else:
            return theta, X_recon, Y_recon

    def _loss(self, X, Y, X_recon, Y_recon, prior_mean, prior_logvar, posterior_mean, posterior_logvar, do_average=True, l1_beta=None, l1_beta_c=None, l1_beta_ci=None, X_recon_neg=None):
        ...
        #Compute -ve triplet loss
        if X_recon_neg != None:
            TL = self.triplet_loss(X_recon, X, X_recon_neg)
            # combine
            loss = NL + KLD + self.triplet_loss_weight * TL
        else:
            loss = NL + KLD
        ...
        # average losses if desired
        if do_average:
            return loss.mean(), NL.mean(), KLD.mean(), TL.mean()
        else:
            return loss, NL, KLD, TL
    ...
    def perturbTheta(x, k):
        x_new =  DecoderNegScholar.perturbTopK(x, k)
