...
class NQTM(object):
    ...
    def encoder(self, x):
        ...
        z_mean = tf.keras.layers.BatchNormalization()(tf.add(tf.matmul(layer_do, weights['out']), biases['out']))

        theta = tf.nn.softmax(z_mean)
        return theta

    def decoder(self, theta):
        x_recon = tf.keras.layers.BatchNormalization()(tf.add(tf.matmul(theta, self.network_weights["weights_gener"]['h2']), 0.0))
