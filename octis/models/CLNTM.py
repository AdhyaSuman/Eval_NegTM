...
class CLNTM(AbstractModel):

    def __init__(self, num_topics=10, lr=2e-3, momentum=0.99, batch_size=64,
                 ...
                 w2v=None, topk=1, top_word=10, model_type='contrastiveScholar',
    ...
    def get_info(self):
        ...
        info['topics'] = CLNTM.show_topic_words(beta, self.vocab, topK=self.top_word)
        return info
    ...
    def _train(self, network_architecture, X, Y, PC, TC, batch_size=200,
        ...
        mb_gen = CLNTM.create_minibatch(X, Y, PC, TC, batch_size=batch_size, rng=rng)
    ...
    def evaluate_perplexity(self, X, Y, PC, TC, batch_size, eta_bn_prop=0.0):
        ...
        for i in range(n_batches):
            batch_xs, batch_ys, batch_pcs, batch_tcs = CLNTM.get_minibatch(X, Y, PC, TC, i, batch_size)
    ...
    def predict_label_probs(self, X, PC, TC, batch_size=200, eta_bn_prop=0.0):
        ...
        for i in range(n_batches):
            batch_xs, batch_ys, batch_pcs, batch_tcs = CLNTM.get_minibatch(X, None, PC, TC, i, batch_size)
