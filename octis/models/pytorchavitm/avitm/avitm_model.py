...
class AVITM_model(object):
    ...
    def __init__(self, input_size, num_topics=10, model_type='prodLDA', hidden_sizes=(100, 100),
                 ...
                 topic_perturb=1, tloss_weight=1, neg_method=2):
        """
        ...
        self.model = DecoderNetwork(
            ...
            topic_perturb=self.topic_perturb, neg_method=self.neg_method)
        ...
        if self.neg_method:
            print("tloss_weight=", self.triplet_loss_weight)

    def _loss(self, inputs, word_dists, prior_mean, prior_variance,
              posterior_mean, posterior_variance, posterior_log_variance, word_dists_neg=None):
        # KL term
        # var division term
        ...
        for batch_samples in loader:
            ...
            if self.neg_method:
                prior_mean, prior_var, posterior_mean, posterior_var, posterior_log_var, \
                    word_dists, topic_words, topic_document, word_dists_neg = self.model(x)
                
                loss = self._loss(x, word_dists, prior_mean, prior_var,
                              posterior_mean, posterior_var, posterior_log_var, word_dists_neg)
            else:
                prior_mean, prior_var, posterior_mean, posterior_var, posterior_log_var, \
    ...
    def _validation(self, loader):
        ...
        for batch_samples in loader:
            ...
            if self.neg_method:
                prior_mean, prior_var, posterior_mean, posterior_var, posterior_log_var, \
                word_dists, topic_word, topic_document, word_dists_neg = self.model(x)
    ...
    def predict(self, dataset):
        ...
        with torch.no_grad():
            for batch_samples in loader:
                ...
                self.model.zero_grad()
                if self.neg_method:
                    _, _, _, _, _, _, _, topic_document, word_dists_neg = self.model(x)
                else:
